...
logging_config = {
    "version": 1,
    "formatters": {
        "brief": {"class": "logging.Formatter", "format": "%(message)s",},
        "default": {"class": "logging.Formatter", "format": "%(levelname)-8s: %(message)s",},
        "verbose": {"class": "logging.Formatter", "format": "%(levelname)-8s %(module)s: %(message)s",},
    },
    "handlers": {"console": {"class": "logging.StreamHandler", "formatter": "default",},},
    "loggers": {"debug": {"level": "DEBUG", "handlers": ["console"],},},
}
...
class TTY():
    ...
    def debugC(self, category: str, *args) -> None:
        if self._debugCategories.get(category) is True:
            self._safeLog(
                "debug", "%s %s [%s]", self._currentWorkerName(), self.format(*args), category,
            )
