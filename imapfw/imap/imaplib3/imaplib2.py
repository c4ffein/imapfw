...
class Request(object):
    ...
    def deliver(self, response):
        ...
        self.ready.set()
        if __debug__:
            self.parent._log(3, "%s:%s.ready.set" % (self.name, self.tag))
...
class IMAP4(object):
    ...
    def __init__(
        self, host=None, port=None, debug=None, debug_file=None, identifier=None, timeout=None, debug_buf_lvl=None
    ):
        ...
        self.tagre = re.compile(br"(?P<tag>" + self.tagpre + br"\d+) (?P<type>[A-Z]+) (?P<data>.*)", re.ASCII)

        if __debug__:
            self._init_debug(debug, debug_file, debug_buf_lvl)
        ...
        try:
            self.welcome = self._request_push(name="welcome", tag="continuation").get_response(
                "IMAP4 protocol error: %s"
            )[1]
            ...
            if __debug__:
                self._log(1, "CAPABILITY: %r" % (self.capabilities,))
    ...
    def ssl_wrap_socket(self):
        ...
        try:
            ...
            TLS_MAP[TLS_COMPAT].update(
                {"ssl23": ssl.PROTOCOL_SSLv23, None: ssl.PROTOCOL_SSLv23,}
            )
            if hasattr(ssl, "PROTOCOL_SSLv3"):  # Might not be available.
                TLS_MAP[TLS_COMPAT].update({"ssl3": ssl.PROTOCOL_SSLv3})
    ...
    def select(self, mailbox="INBOX", readonly=False, **kw):
        ...
        try:
            ...
            if typ != "OK":
                ...
                if typ == "BAD":
                    self._deliver_exc(
                        self.error, "%s command error: %s %s. Data: %.100s" % (name, typ, dat, mailbox), kw
                    )
                return self._deliver_dat(typ, dat, kw)
            self.state = SELECTED
            if __debug__:
    ...
    def _append_untagged(self, typ, dat):
        ...
        if __debug__:
            self._log(5, 'untagged_responses[%s] %s += ["%.80r"]' % (typ, len(urd) - 1, dat))
        ...
        for arg in args:
            if arg is not None:
                if isinstance(arg, dflttyp):
                    return arg
                if __debug__:
                    self._log(0, "bad arg is %s, expecting %s" % (type(arg), dflttyp))
        return dflt
    ...
    def _command(self, name, *args, **kw):
        ...
        if __debug__:
            self._log(3, "state_change_pending.acquire")
        ...
        if cmdtyp == "async":
            ...
            if __debug__:
                self._log(3, "state_change_pending.release")
        else:
            ...
            if need_event:
                if __debug__:
                    self._log(3, "sync command %s waiting for empty commands Q" % name)
                self.state_change_free.wait(threading.TIMEOUT_MAX)
                if __debug__:
                    self._log(3, "sync command %s proceeding" % name)
        ...
        while True:
            ...
            if __debug__:
                self._log(4, "continuation => %s, %r" % (ok, data))
    ...
    def _command_completer(self, cb_arg_list):
        ...
        if typ == "BAD":
            ...
            return
        if __debug__:
            self._log(4, "_command_completer(%s, %s, None) = %s" % (response, cb_arg, rqb.tag))
        if "untagged_response" in kw:
    ...
    def _get_untagged_response(self, name, leave=False):
        ...
        for i, (typ, dat) in enumerate(self.untagged_responses):
            if typ == name:
                ...
                self.commands_lock.release()
                if __debug__:
                    self._log(5, "_get_untagged_response(%s) => %.80r" % (name, dat))
                return dat
    ...
    def _put_response(self, resp):
        ...
        if self._expecting_data:
            ...
            if __debug__:
                self._log(5, "_put_response expecting data len %s, got %s" % (self._expecting_data_len, rlen))
        ...
        resp = resp[:-2]
        if __debug__:
            self._log(5, "_put_response(%r)" % resp)
        ...
        if self._literal_expected is not None:
            ...
            if self._match(self.literal_cre, dat):
                ...
                if __debug__:
                    self._log(4, "expecting literal size %s" % self._expecting_data_len)
                return
            ...
            if self._match(self.tagre, resp):
                ...
                if not tag in self.tagged_commands:
                    if __debug__:
                        self._log(1, "unexpected tagged response: %r" % resp)
                else:
                ...
                if self.mo is None:
                    ...
                    if self._match(self.continuation_cre, resp):
                        if not continuation_expected:
                            if __debug__:
                                self._log(1, "unexpected continuation response: '%r'" % resp)
                            return
                        self._request_pop("continuation", (True, self.mo.group("data")))
                        return

                    if __debug__:
                        self._log(1, "unexpected response: '%r'" % resp)
                    return
                ...
                if self._match(self.literal_cre, dat):
                    ...
                    self._expecting_data_len = int(self.mo.group("size"))
                    if __debug__:
                        self._log(4, "read literal size %s" % self._expecting_data_len)
        ...
        if typ in ("NO", "BAD", "BYE"):
            if typ == "BYE":
                self.Terminate = True
            if __debug__:
                self._log(1, "%s response: %r" % (typ, dat))
    ...
    def _request_pop(self, name, data):
        ...
        if __debug__:
            self._log(4, "_request_pop(%s, %r) [%d] = %s" % (name, data, len(self.tagged_commands), rqb.tag))
        rqb.deliver(data)

        if need_event:
            if __debug__:
                self._log(3, "state_change_free.set")
    ...
    def _request_push(self, tag=None, name=None, **kw):
        ...
        self.commands_lock.release()
        if __debug__:
            self._log(4, "_request_push(%s, %s, %s) = %s" % (tag, name, repr(kw), rqb.tag))
        return rqb
    ...
    def _handler(self):
        ...
        if __debug__:
            self._log(1, "terminating: %s" % repr(val))
                    ...
                    if state & select.POLLIN:
                        ...
                        while True:
                            ...
                            if stop < 0:
                                ...
                                break
                            stop += 1
                            line_part, start, line = b"", stop, line_part + data[start:stop]
                            if __debug__:
                                self._log(4, "< %r" % line)
                            self.inq.put(line)
                ...
                try:
                    ...
                    while True:
                        ...
                        if stop < 0:
                            ...
                            break
                        stop += 1
                        line_part, start, line = b"", stop, (line_part + data[start:stop]).decode(errors="ignore")
                        if __debug__:
                            self._log(4, "< %r" % line)
                        self.inq.put(line)
                        if self.TerminateReader:
...
class IMAP4_stream(IMAP4):
    ...
    def open(self, host=None, port=None):
        ...
        if __debug__:
            self._log(0, 'opening stream from command "%s"' % self.command)
...
if __name__ == "__main__":
    ...
    try:
        ...
        for cmd, args in test_seq2:
            ...
            if not uid:
                continue
            run("uid", ("FETCH", uid[-1], "(FLAGS INTERNALDATE RFC822.SIZE RFC822.HEADER RFC822.TEXT)"))
